...
usage: scripts/wt.sh <command>
  ...
  new <branch> [base]   create branch off [base] (default: dev, else main) + worktree
  add <branch>          worktree for an existing branch (e.g. dev, test)
  ls                    list worktrees
...
default_base() {
  if git show-ref --verify --quiet refs/heads/dev; then echo dev
  elif git ls-remote --exit-code --heads origin dev >/dev/null 2>&1; then echo origin/dev
  else echo main
  fi
}
